...
class QuestionForm(forms.Form):
    # Form Field -> 질문 입력
    # 변수명 (요청파라미터 name) = FormField(): 어떤 값을 입력받을지 타입.

    # Form Field 1개 -> 한 개 이름의 요청파라미터 입력 태그를 설정.
    ## 같은 이름으로 여려개 입력을 받을 경우 -> Form set을 이용해서 구현.
    question_text = forms.CharField( # 문자열 입력 폼 - View에서 문자열로 읽기.
        ...
        widget=forms.TextInput(             # widget은 input 태그를 지정. type=text
            attrs={"class":"form-control"}  # 입력 tag의 attribute 설정.
        )
    )
        # 검증 메소드를 추가. -> 업무규칙의의한 검증
