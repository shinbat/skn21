...
select * from emp where salary < (select avg(salary) from emp ) order by salary desc;
...
select * from emp where mgr_id in (
...
select * from emp where salary > (
select min(salary) from emp where emp_id in (101,102,103))
order by salary ;
...
select * from emp where salary > all (select max(salary) from emp where dept_id = 20)