...
select CHAR_LENGTH(emp_name), CHARACTER_LENGTH(emp_name ) from emp;

-- 직원 이름(emp_name) 의 자릿수를 15자리로 맞추고 15자가 안되는 이름의 경우  공백을 앞에 붙여 조회. 
select rPAD(emp_name, 15,' '), CHAR_LENGTH(LPAD(emp_name, 15,' ')) from emp;
...
select emp_id, emp_name, salary, salary * 1.15 , ceil(salary * 1.15), floor(salary * 1.15)  from emp;

select truncate(12345.56789, 2);
-- 위의 SQL문에서 인상 급여(sal_raise)와 급여(salary) 간의 차액을 추가로 조회 
-- (직원ID(emp_id), 이름(emp_name), 15% 인상급여, 인상된 급여와 기존 급여(salary)와 차액)
select emp_id, emp_name, salary, salary * 1.15 , format(salary * 1.15, 0)  from emp;
...
-- 날짜 타입에서 년 월 일 조회
select hire_date , adddate(hire_date , interval 3 quarter) from emp;
-- 시간 타입에서 시 분 초 조회

-- 특정 기간 만큼 전,후의 일시를 조회
select hire_date, datediff(now(), hire_date) from emp;

-- EMP 테이블에서 부서이름(dept_name)이 'IT'인 직원들의 '입사일(hire_date)로 부터 10일전', 입사일, '입사일로 부터 10일 후' 의 날짜를 조회. 
select hire_date, subdate(hire_date, interval 10 day) from emp;
;
-- ID(emp_id)가 200인 직원의 이름(emp_name), 입사일(hire_date)를 조회. 입사일은 yyyy년 mm월 dd일 형식으로 출력.
select hire_date, date_format(hire_date, '%Y년 %m월 %d일') from emp;

--  각 직원의 이름(emp_name), 근무 개월수 (입사일에서 현재까지의 달 수)를 계산하여 조회. 근무개월수 내림차순으로 정렬.
select emp_name, hire_date, timestampdiff(month, hire_date , now() ) as '근무개월수'
from emp
order by 3 desc;
...
select emp_id, emp_name, job, ifnull(dept_name, '배치전') from emp where dept_name is null;

-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 커미션 (salary * comm_pct)을 조회. 커미션이 없는 직원은 0을 출력.
select emp_id, emp_name, salary, ifnull(salary * comm_pct , 0) from emp where comm_pct is null;
